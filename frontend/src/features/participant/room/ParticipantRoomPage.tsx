...
import { AppShell, ConnectionBanner } from "../../../components/layout";
import {
  Button,
  EmptyState,
  Icon,
...
function analysisMessage(phase?: string | null): { title: string; body: string } {
  if (phase === "complete") {
    return {
      title: "Results ready",
      body: "Junto is opening your discussion group now.",
    };
  }
  return {
...
export function ParticipantRoomPage() {
  ...
  usePolling(
    ...
    {
      enabled: Boolean(
        room && room.status !== "published" && room.status !== "failed",
      ),
  ...
  useEffect(() => {
    ...
    api.getMyGroup(roomId).then(setGroup).catch((reason: unknown) => setStatusError(readableError(reason)));
  ...
  if (room.status === "analyzing") {
    const message = analysisMessage(room.analysisPhase);
    return (
      <AppShell context={room.title}>
  ...
  return (
    <div className={styles.runnerShell}>
      ...
      <main id="question-content" className={styles.runnerMain}>
        {!onReviewPage && currentQuestion ? (
          <section className={styles.questionPage} aria-labelledby={`question-${currentQuestion.id}`}>
            ...
            <TextArea
              ...
              placeholder="Write your response here. Explain your reasoning in enough detail for another person to follow it."
          ...
          <section className={styles.reviewPage} aria-labelledby="submission-review-title">
            ...
            <p>
              You answered {answeredCount} of {questions.length} questions. You can return to any question before submitting.
            </p>
        ...
        {error && !onReviewPage ? (
          <InlineNotice tone="error" title="Answer not saved">
            {error}
          </InlineNotice>
        ) : null}
      </main>
      ...
      <nav className={styles.runnerNavigation} aria-label="Question navigation">
        ...
        <span className={styles.progressText}>{answeredCount} answered</span>
      </nav>
    </div>
  );
}
...
function SaveIndicator({ state }: { state: SaveState }) {
  if (state === "saving") return <span className={styles.saveIndicator} role="status" aria-live="polite">Saving…</span>;
  if (state === "error") return <span className={styles.saveError} role="status" aria-live="polite">Not saved</span>;
  if (state === "saved") {
    return (
  ...
  return <span className={styles.saveIndicator} role="status" aria-live="polite">Editing</span>;
}
...
function WaitingRoom({
  ...
  return (
    <div className={styles.waitingRoom}>
      ...
      <h1>{title}</h1>
      <p>{body}</p>
      {analyzing ? <div className={styles.waitingLine} aria-hidden="true"><span /></div> : null}
    </div>
  );
}

function ParticipantGroupView({
  result,
  error,
}: {
  result: MyGroupResponse | null;
  error: string | null;
}) {
  if (!result) {
    return (
  ...
  return (
    <div className={styles.groupPage}>
      ...
      <InlineNotice tone="info" title="Start the discussion">
        Introduce yourselves, then take each question in order. Ask what evidence or reasoning led each person to their answer.
      </InlineNotice>
    </div>
  );
}
