...
def test_complete_lobby_questionnaire_and_placeholder_group_flow(
  ...
  participants = [join_participant(app, join_code, name) for name in ("Maya", "Alex", "Noor", "Sam")]
...
def test_deadline_claims_analysis_once_and_freezes_answers(
  backend: BackendFixture,
) -> None:
  ...
  assert scheduler.run_ready() == 2
  published = app.state.room_service.get_room(UUID(room_id))
  assert published.status == "published"
  assert published.grouping_result is not None
  assert published.grouping_result.trigger == "deadline"


def test_open_requires_host_approved_coverage_units(backend: BackendFixture) -> None:
...
def test_room_capabilities_prevent_cross_session_host_access(
  backend: BackendFixture,
) -> None:
  app, _scheduler, _clock = backend
  host = TestClient(app)
