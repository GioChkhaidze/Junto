...
class MaterialUploadLimitMiddleware:
  """Bound multipart reference requests before Starlette spools an uploaded file."""
  ...
  def _is_material_upload(scope: Scope) -> bool:
    ...
    return path == "/api/authoring/suggestions" or path.startswith("/api/rooms/") and path.endswith("/materials")
    ...
    response = JSONResponse(
      ...
      content={
        "error": {
          ...
          "message": "The reference upload request is too large.",
          "details": {},
        }
      },
    )
